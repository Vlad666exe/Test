...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		// "GLOBAL" SCRIPTS
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		for (file in Assets.list().filter(folder -> folder.contains('assets/shared/scripts/'))) // by idklool
			{
				#if LUA_ALLOWED
				if(file.toLowerCase().endsWith('.lua'))
					new FunkinLua(Asset2File.getPath(file));
				#end
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		for (file in Assets.list().filter(folder -> folder.contains('assets/shared/data/$songName/'))) // by idklool
		{
				#if LUA_ALLOWED
				if(file.toLowerCase().endsWith('.lua'))
					new FunkinLua(Asset2File.getPath(file));
	...
	function startCharacterScripts(name:String)
	{
		...
		if(doPush)
		{
			...
			if(doPush) new FunkinLua(Asset2File.getPath(luaFile));
		}
		#end
	...
	public function startLuasNamed(luaFile:String)
	{
		...
		#end
		{
			...
			new FunkinLua(Asset2File.getPath(luaToLoad));
			return true;
		}
		return false;
	}
