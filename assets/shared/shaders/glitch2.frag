	...
	void main()
	{
		...
		for(float i=0.;i<10.*AMT;i+=.25){
			float thingy = i;
			float sliceY=random2d(vec2(floor(iTime*SPEED*60),2345.+thingy));
			float sliceH=random2d(vec2(floor(iTime*SPEED*60),925.+thingy))*.075;
			float hOffset=randomRange(vec2(floor(iTime*SPEED*60),25.+thingy),-maxOffset,maxOffset);
